...
std::string Base64Helper::encode(const char * in_str,size_t len)
{
    std::string out_str;
...
std::vector<char> Base64Helper::decode(const std::string in_str)
{
    ...
    while ( i<len)
    {
		...
		out_chars.push_back(char( (c1<<2) | ((c2&0x30)>>4) ));
		...
		out_chars.push_back(char( ((c2&0XF)<<4) | ((c3&0x3C)>>2) ));
		...
		out_chars.push_back(((c3&0x03)<<6) | c4 );
    }
