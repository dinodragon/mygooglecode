					...
					PSTR szCmdLine, int iCmdShow)
{
	...
	hAccel = LoadAccelerators (hInstance, szAppName) ; //function loads the specified accelerator table. 


	//特殊的消息循环结构.估计是将子窗口消息送到子窗口.
...
//调用示例:OkMessage (hwnd, TEXT ("Could not write file %s"),szTitleName) ;
...
LRESULT CALLBACK WndProc (HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	{
	case WM_CREATE:
		hInst = ((LPCREATESTRUCT) lParam) -> hInstance ;  //取得应用程序实例号
		...
		//给Edit控件发送消息,设置最大文本长度.在本项目中没找到处理该消息的消息处理函数.
		SendMessage(hwndEdit,EM_LIMITTEXT,32000,0L) ;

		// Initialize common dialog box stuff

		PopFileInitialize (hwnd) ;//函数位于PopFile中.初使化
